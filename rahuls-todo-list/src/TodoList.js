  ...
  <div className="p-4 bg-success rounded">
    <Form>
      <Form.Group className="mb-3">
        <Form.Label className="text-white" htmlFor="todoItem">ToDo Item</Form.Label>
        <Form.Control type="text" id="todoItem" placeholder="Add todo item" />
      </Form.Group>
      <Form.Group className="mb-3">
                ...
                <ListGroup>
                  {todos.map((todo, index) => (
                    <ListGroup.Item
                      ...
                      href={`#link${index}`}
                      key={index}
                      className={`list-group-item-${getVariant(index)} text-white`}  // Updated className
                    >
                      {todo.title}
                    </ListGroup.Item>
                  ))}
